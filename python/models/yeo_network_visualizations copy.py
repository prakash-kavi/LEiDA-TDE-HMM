...
def visualize_network_timeseries(model_data, group, k, subject_idx=0, max_timepoints=360):
    """
    Create visualization of network time series with state overlay for a single subject.
    """
    ...
    # Create figure with gridspec for network data and state assignments
    fig = plt.figure(figsize=(16, 10))
    gs = gridspec.GridSpec(2, 1, height_ratios=[4, 1], hspace=0.05)
    
    # Network time series plot
    ax1 = plt.subplot(gs[0])
    
    # Plot each network's time series
    network_names = model_data['network_fields']
    for i, name in enumerate(network_names):
        plt.plot(network_z[:, i], label=f"{name} ({NETWORK_NAMES.get(name, name)})", 
                 linewidth=1.5, alpha=0.8)
    
    plt.ylabel('Z-score')
    plt.title(f'{group.capitalize()} - Subject {subject_idx+1} Network Activity with States (k={k})')
        ...
        for t, state in enumerate(vpath_scalar):
            if state != prev_state or t == len(vpath_scalar) - 1:
                if prev_state >= 0:
                    ...
                    if t - start_pos > max_timepoints / 20:  # Only label segments that are visible
                        ax2.text(
    ...
    plt.xlim(0, len(network_data))
    plt.ylim(0, 1)
    plt.ylabel('State')
    plt.xlabel('Time (TR)')
    plt.yticks([])  # Remove y ticks for state plot
    
    # Create a second visualization: heatmap of network activity with state overlay
    fig2 = plt.figure(figsize=(16, 8))
    
    # First create a heatmap of network activity
    ax3 = plt.subplot(111)
    ...
    if vpath_subject is not None:
        # Use the scalar vpath created earlier
        prev_state = vpath_scalar[0]
        for t in range(1, len(vpath_scalar)):
            if vpath_scalar[t] != prev_state:
                plt.axvline(x=t, color='black', linestyle='-', linewidth=1, alpha=0.5)
                prev_state = vpath_scalar[t]
    
    # Label axes
    plt.xlabel('Time (TR)')
    plt.ylabel('Network')
    plt.yticks(range(len(network_names)), [f"{name}" for name in network_names])
    plt.title(f'{group.capitalize()} - Subject {subject_idx+1} Network Activity Heatmap (k={k})')
    
    # Add state labels at the top
    if vpath_subject is not None:
        # Find state change points using scalar vpath
        change_points = [0]
        states = [vpath_scalar[0]]
        
        for t in range(1, len(vpath_scalar)):
            if vpath_scalar[t] != vpath_scalar[t-1]:
                change_points.append(t)
                states.append(vpath_scalar[t])
        
        # Add one final change point at the end
        change_points.append(len(vpath_scalar))
        
        # Add state labels
        for i in range(len(states)):
            # Only add label if segment is long enough
            if change_points[i+1] - change_points[i] > max_timepoints / 15:
                ax3.text(
                    (change_points[i] + change_points[i+1]) / 2, 
                    -0.5,  # Just above the heatmap
                    f"State {states[i] + 1}",
                    ha='center', va='center',
                    fontsize=10, fontweight='bold',
                    bbox=dict(
                        boxstyle="round,pad=0.3",
                        fc=state_colors[states[i]],
                        ec="black",
                        alpha=0.7
                    )
                )
    ...
    # Save both figures
    output_path1 = os.path.join(output_dir, f'{group}_k{k}_subject{subject_idx+1}_network_timeseries.png')
    output_path2 = os.path.join(output_dir, f'{group}_k{k}_subject{subject_idx+1}_network_heatmap.png')
    
    save_figure(fig, output_path1)
        ...
        for group in ['controls', 'meditators']:
            ...
            for k in k_values:
                ...
                for subject_idx in range(min(2, len(model_data['indices']))):
                    # Create line plot with state overlay
                    visualize_network_timeseries(model_data, group, k, subject_idx)
