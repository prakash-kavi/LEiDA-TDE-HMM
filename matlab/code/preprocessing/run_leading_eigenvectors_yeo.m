%% LEiDA Computation for Yeo Networks
% Computes leading eigenvectors from Hilbert-transformed data
% Processes data at Yeo network level (7 networks + subcortical)
% Input: hilbert_data.mat containing hilbert_med and hilbert_con
% Output: eigenvectors_data_yeo.mat containing network-level phase-based eigenvectors
...
if ~all(isfield(networks, network_fields))
    error('Missing network definitions in define_network_mappings()');
end
...
for g = 1:length(groups)
    group = groups{g};
    fprintf('Computing Yeo network eigenvectors for %s...\n', group);
...
% Save results
output_file = fullfile(PROCESSED_DIR, 'eigenvectors_data_yeo.mat');
save(output_file, 'eigenvectors_data_yeo');
fprintf('Network-level leading eigenvector computation complete.\n');
