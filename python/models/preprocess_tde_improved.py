...
RESULTS_DIR = os.path.join(ROOT_DIR, 'results', 'visualizations', 'tde_revised')
...
def visualize_embedded_data(data, group_data, group, subject_idx=0, standardize_method='bygroup'):
    """Visualize original and TDE-processed data for a subject with accurate color scaling and balanced colors"""
    logger.info(f"Visualizing {group} subject {subject_idx+1} data with {standardize_method} standardization...")
    ...
    # Calculate data statistics for all three datasets
    orig_min, orig_max = np.min(orig_seq), np.max(orig_seq)
    orig_abs_max = max(abs(orig_min), abs(orig_max))
    preproc_min, preproc_max = np.min(preproc_seq), np.max(preproc_seq)
    preproc_abs_max = max(abs(preproc_min), abs(preproc_max))
    emb_min, emb_max = np.min(embedded_seq), np.max(embedded_seq)
    emb_abs_max = max(abs(emb_min), abs(emb_max))
    
    # Log data ranges for diagnostics
    logger.info(f"Original data range: [{orig_min:.2f}, {orig_max:.2f}], abs max: {orig_abs_max:.2f}")
    logger.info(f"Preprocessed data range: [{preproc_min:.2f}, {preproc_max:.2f}], abs max: {preproc_abs_max:.2f}")
    logger.info(f"Embedded data range: [{emb_min:.2f}, {emb_max:.2f}], abs max: {emb_abs_max:.2f}")
    
    # Create figure with 3 plots: original, preprocessed, and embedded
    plt.figure(figsize=(18, 16))  # Taller to accommodate histograms
    
    # Plot 1: Original data with symmetric scaling
    plt.subplot(3, 2, 1)  # 3 rows, 2 columns, position 1
    plt.imshow(orig_seq.T, aspect='auto', cmap='coolwarm', 
               vmin=-orig_abs_max, vmax=orig_abs_max)  # Symmetric scaling centered at 0
    ...
    plt.colorbar(label=f'Eigenvector Value [-{orig_abs_max:.2f}, +{orig_abs_max:.2f}]')
    
    # Histogram for original data
    plt.subplot(3, 2, 2)
    plt.hist(orig_seq.flatten(), bins=50, color='gray', alpha=0.7)
    plt.axvline(x=0, color='black', linestyle='--')
    plt.title('Histogram of Original Values')
    plt.xlabel('Value')
    plt.ylabel('Frequency')
    
    # Plot 2: Standardized data with adjusted scale
    plt.subplot(3, 2, 3)
    plt.imshow(preproc_seq.T, aspect='auto', cmap='coolwarm',  # Use coolwarm for better perceptual balance
              vmin=-preproc_abs_max, vmax=preproc_abs_max)  # Symmetric scaling
    plt.title(f'Standardized Data (Method="{standardize_method}")')
    plt.xlabel('Time (TRs)')
    plt.ylabel('Network')
    plt.yticks(range(len(data['network_fields'])), data['network_fields'])
    plt.colorbar(label=f'Z-score [-{preproc_abs_max:.2f}, +{preproc_abs_max:.2f}]')
    
    # Histogram for standardized data
    plt.subplot(3, 2, 4)
    plt.hist(preproc_seq.flatten(), bins=50, color='gray', alpha=0.7)
    plt.axvline(x=0, color='black', linestyle='--')
    plt.title('Histogram of Standardized Values')
    plt.xlabel('Value')
    plt.ylabel('Frequency')
    
    # Plot 3: Embedded data with appropriate symmetric scaling
    plt.subplot(3, 2, 5)
    plt.imshow(embedded_seq.T, aspect='auto', cmap='coolwarm',  # Use coolwarm for better perceptual balance
               vmin=-emb_abs_max, vmax=emb_abs_max)  # Symmetric scale based on data
    
    # Calculate number of components
    n_components = embedded_seq.shape[1]
    plt.title(f'TDE-Embedded Data with PCA ({n_components} components)')
    plt.xlabel('Time (TRs)')
    plt.ylabel('PCA Component')
    plt.colorbar(label=f'PCA Component Value [-{emb_abs_max:.2f}, +{emb_abs_max:.2f}]')
    
    # Histogram for embedded data
    plt.subplot(3, 2, 6)
    plt.hist(embedded_seq.flatten(), bins=50, color='gray', alpha=0.7)
    plt.axvline(x=0, color='black', linestyle='--')
    plt.title('Histogram of PCA Component Values')
    plt.xlabel('Value')
    plt.ylabel('Frequency')
    ...
    plt.figtext(0.5, 0.01, 
                ...
                "Histograms show value distributions; skewness causes color imbalance.",
...
def main():
    ...
    try:
        ...
        if not args.skip_visualization:
            ...
            for subj_idx in [0, 5, 10, 15]:  # Example subject indices for visualization
                if subj_idx < len(data['controls_sequences']):
                    visualize_embedded_data(data, controls_tde, 'controls', subj_idx, args.standardize)
                    visualize_state_time_course(data, controls_tde, 'controls', subj_idx)
                
                if subj_idx < len(data['meditators_sequences']):
                    visualize_embedded_data(data, meditators_tde, 'meditators', subj_idx, args.standardize)
